...
type metricsMonitoring interface {
	...
	ObserveModelResponseLatency(modelID string, latency time.Duration)
	UpdateModelResponseRequest(modelID string, c int)
	ObserveRequestCount(modelID, tenantID string, statusCode int32)
}

type taskSender interface {
	SendChatCompletionTask(ctx context.Context, tenantID string, req *v1.CreateChatCompletionRequest, header http.Header) (*http.Response, *infprocessor.ProcessingStats, error)
